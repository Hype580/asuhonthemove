import { saveNannies, generateId } from './storage';
import type { Nanny } from '@/types/nanny';

// Existing hardcoded nannies data
const existingNannies = [
  {
...
export async function migrateExistingNannies(): Promise<void> {
  try {
    // Add IDs to existing nannies
    const nanniesWithIds: Nanny[] = existingNannies.map(nanny => ({
      ...nanny,
      id: generateId()
    }));

    await saveNannies(nanniesWithIds);
